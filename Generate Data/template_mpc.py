...
from casadi import *            # To compute gradients, Jacobian and Hessians 
...
def template_mpc(model, silence_solver = False):
    ...
    --------------------------------------------------------------------------
    """
    mpc = do_mpc.controller.MPC(model)               # Declaration of variable 'mpc' as a "do_mpc.controller.MPC" type
    ...
    mpc.settings.nlpsol_opts = {'ipopt.linear_solver': 'MA27', 'ipopt.hsllib': '/home/martinxavier/coinhsl/.libs/libcoinhsl.so'}
    ...
    def tvp_fun(t_now):
        ...
        if ((np.round(np.squeeze(t_now),3)+epsilon)%T_REF) < T_REF/2: 
            # Select seed
            seed((np.round(np.squeeze(t_now),3)+epsilon)//T_REF + 150) 

            # Get the reference
            tvp_template['_tvp'] = 0.9*random()
        else: 
            # Select seed
            seed((np.round(np.squeeze(t_now),3)+epsilon)//T_REF + 10**6) 
