...
class SequenceDataset(Dataset):
    ...
    Parameters:
        dataframe (pd.DataFrame): The DataFrame containing the column to shift.
        target (list of str): The target column in the dataset.
        features (list of str): The list of feature columns (independent variables) used to predict the target.
...
class FNNModel(nn.Module):
    ...
    Parameters:
        ...
        width_dim (int): Number of hidden layers.
        activation_fn (callable, optional): Activation function (default is Tanh).
        bias (bool, optional): Whether to use bias in layers (default is False).
    
    Returns:
        out (torch.Tensor): 
            Neural network prediction with shape (batch_size, output_dim) constrained between [−1,1].
    """
...
class Data:
    ...
    def random_simulation(seed:int):
        ...
        Parameters:
            seed (int): The seed value to use for random number generation, ensuring reproducibility 
    ...
    def get_scaler(scaler:str):
        ...
        Parameters:
            scaler (str): The name of the scaler to retrieve. Supported options:
                        'minmax' -> 'MinMaxScaler'.
        ...
        Returns:
            scalers[scaler_name] : 
                An instance of the requested scaler class:
                    - 'MinMaxScaler'.
                    - 'StandardScaler'.
                    - 'MaxAbsScaler'.
                    - 'RobustScaler'.
                    - 'None' if an invalid scaler name is provided.
        """
        scalers = {
            "minmax":   MinMaxScaler,
            "standard": StandardScaler,
    ...
    @staticmethod
    def get_individual_dataset(df:pd.DataFrame, target:list, features:list, t_traj:int):
        """
        ...
        for i in range (len(df) // t_traj):
            ...
            # Regular DataSets
            datasets.append(SequenceDataset(data, target, features)) 
...
class NeuralNetwork:
    ...
    def train_model(data_loader:DataLoader, model:nn.Module, loss_function:nn.Module, optimizer: torch.optim.Optimizer, device:torch.device):
        ...
        Parameters:
            ...
            device (torch.device): 
                Device in which training is performed (gpu or cpu).
    ...
    @staticmethod
    def train_loop(controller:nn.Module, train_loader:DataLoader, val_loader:DataLoader, loss_function:nn.Module, optimizer:torch.optim.Optimizer, n_epochs:int, device:torch.device):
        """
        Loop to training a neural network using unsupervised learning.
        ...
        """
        logger.info("\n-------- \nTRAINING \n--------")
        validation_loss = NeuralNetwork.validate_model(val_loader, controller, loss_function, device)
        ...
        with alive_bar(n_epochs, title='TRAIN', enrich_print = False) as bar:
            for ix_epoch in range(n_epochs):
                ...
                # Training function
                training_loss = NeuralNetwork.train_model(train_loader, controller, loss_function, optimizer, device)
        ...
        return controller, vec_t_loss, vec_v_loss, comp_time
    ...
    def loop(N_traj:int, T_traj:int, Ts:float, controller:nn.Module, simulator:do_mpc.simulator.Simulator, 
             init_state:dict, scalers:dict, bias_work:float, bias_return:float, bar_title:str, process_std: np.ndarray, meas_std: np.ndarray,
             feasibility = False):
        """
        ...
        with alive_bar(N_traj, title = bar_title, enrich_print = False) as bar:
            for idx in range(N_traj):
                ...
                # Change shape of initial condition
                x0 = np.expand_dims(x0, axis=1)
                warm_start = {'u': np.zeros(3), 'lam_g': np.zeros(6)}
        ...
        results = {
            'y' : dataset_y  , 'y_dot': dataset_y_dot, 
        ...
        return simulator, results, timer, feasibility_results
...
class FeasibilityRecovery:
    ...
    def feasibility_recover(u_NN:np.float64, x_init:np.ndarray, warm_start: dict, feasibility_variables:dict):
        ...
        Parameters:
            ...
            feasibility_variables (dict): dictionary to get the optimization details:
                    ...
                    * 'slack_variables' (casadi.MX): slack variables.
        ...
        opti = feasibility_variables.get('optimization')

        # Get optimization parameters
        network_command = feasibility_variables.get('u_param')
        initial_state = feasibility_variables.get('x_param')

        # Get optimization variable
        u = feasibility_variables.get('opti_var')

        # Get slack variables
        s = feasibility_variables.get('slack_variables')
    ...
    @staticmethod
    def NN_make_step(X:np.ndarray, model:nn.Module, scalers:dict, x_init:np.ndarray, warm_start:dict, feasibility:dict = None):
        """
        Function to perform a step forward (control prediction) using the neural network as a controller.
    ...
    def forging_model(x:MX, u:MX):
        """
        Function to compute the ODE of the open-die forging problem.
        ...
        b1 = B0*(1 + 0.67*(H0/h1*W0/w1 - 1))            # Deformed bite lenght [m]
        ...
        # Pressure saturation
        # p1 = if_else(p1 >= 0, p1, 0)
        # p2 = if_else(p2 >= 0, p2, 0)
