...
def template_mpc(model, silence_solver = False):
    ...
    ref = _tvp['ref']               # Vitesse desirée
    mterm = (_x['y_dot'] - ref)**2  # Terminal cost 
    lterm = (_x['y_dot'] - ref)**2  # Stage cots 

    # Set objective function
    mpc.set_objective(mterm = mterm, lterm = lterm) 
    mpc.set_rterm(u = 0.02) # The penalization is in its quadratique form 
    ...
    def tvp_fun(t_now):
        ...
        if ((np.round(np.squeeze(t_now),3)+epsilon)%T_REF) < T_REF/2: 
            # Select seed
            seed((np.round(np.squeeze(t_now),3)+epsilon)//T_REF + 150 + 100) 

            # Get the reference
            tvp_template['_tvp'] = 0.8*random() + 0.1
        else: 
            # Select seed
            seed((np.round(np.squeeze(t_now),3)+epsilon)//T_REF + 10**6 + 100) 
