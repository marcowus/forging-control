...
def template_model(symvar_type='SX'):
    ...
    # Minimum physically admissible absolute pressure (use PT or vapor pressure)
    P_MIN = 0  # or e.g., P_MIN = 2e3

    # C^1 softplus-like ReLU: max(x,0) ≈ 0.5*(x + sqrt(x^2 + eps))
    def smooth_relu(x, eps=1e-6):
        return 0.5*(x + sqrt(x*x + eps))
    
    # Floor pressures used everywhere downstream (flows, leakage, RHS, measurements)
    p1_eff = P_MIN + smooth_relu(p1 - P_MIN)
    p2_eff = P_MIN + smooth_relu(p2 - P_MIN)

    # p1_eff = if_else(p1_eff <= PS, p1_eff, PS)
    # p2_eff = if_else(p1_eff <= PS, p2_eff, PS)

    ############ Oil flow in the servo-valves ############

    # Working Cylinders
    qvPB_work = np.pi*D*z*CD*sqrt(2/RHO*fabs(PS - p1_eff))*sign(PS - p1_eff)
    qvAT_work = np.pi*D*z*CD*sqrt(2/RHO*fabs(p2_eff - PT))*sign(p2_eff - PT)
    
    # Return Cylinders
    qvPB_return = np.pi*D*z*CD*sqrt(2/RHO*fabs(p1_eff - PT))*sign(p1_eff - PT)
    qvAT_return = np.pi*D*z*CD*sqrt(2/RHO*fabs(PS - p2_eff))*sign(PS - p2_eff)
    ...
    # Time varying variable - Reference
    ...
    model.set_rhs('y_dot', (3*np.pi*D1**2*p1_eff/4 - np.pi*D2**2*p2_eff/2 - B*y_dot - Ft - Fd_article)/M + G, process_noise=True) # Global
    model.set_rhs('p1'   , (KB/V1*(qvPB/3 - A1*y_dot - KL_1*p1_eff)), process_noise=True)
    model.set_rhs('p2'   , (KB/V2*(-qvAT/2 + A2*y_dot - KL_2*p2_eff)), process_noise=True)
    ...
    model.set_meas('p1_meas'   , p1_eff, meas_noise=True)
    model.set_meas('p2_meas'   , p2_eff, meas_noise=True)
